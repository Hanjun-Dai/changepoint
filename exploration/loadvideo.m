function [img_sequence, video_path] = loadvideo(folder_name, filetype)
    folder = fullfile(folder_name, filetype);
    image_names = dir(folder);
    image_names = {image_names.name};
    image_names = sort(image_names);
    if nargout > 1
        video_path = fullfile(folder_name, 'video.avi');
    ...
    for i = 1 : length(image_names)
        ...
        img = imread(fullfile(folder_name, image_names{i}));
        if i == 1
